...
func main() {
	...
	// Start health monitoring logic
	kubeConfigPath := filepath.Join(os.Getenv("HOME"), ".kube", "config") // MIGHT NEED TO CHANGE THIS
	...
	err = controllers.StartHealthMonitoring(kubeConfigPath, azureCfg)
	if err != nil {
		fmt.Printf("Error starting health monitoring: %v\n", err)
		return
	}
...
/*
	...
	WILL NEED TO DO THE WAITCACHEFORSYNC LATER IN MAIN FUNCTION. WE ARE STARTING GO ROUTINES WITHIN HEALTH_CONTROLLER.GO

	NEED TO WRITE SELECT {} AT END OF MAIN FUNCTION TO KEEP IT RUNNING
*/
